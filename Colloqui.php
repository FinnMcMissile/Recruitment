	...
	<body>
		...
		<div class="container">
			<div class="offset-md-1 mt-2">
				...
				<a href="./Convoca_candidato.php">Aggiungi <i class="fa fa-user-plus"></i></a>
			</div>
				...
				<table class="table">
					...
					<tbody>
						<?php
							...
							$stringa_query="SELECT ID_colloquio, Data, Ora, Candidati.Nome AS Nome_candidato, Candidati.Cognome AS Cognome_candidato, Utenti.Nome AS Nome_esaminatore, Utenti.Cognome AS Cognome_esminatore, Metodo_colloquio, Esito FROM colloqui JOIN Candidati ON Colloqui.ID_candidato=Candidati.ID_candidato JOIN Utenti ON Colloqui.ID_utente=Utenti.ID_utente";
							$risultato=$connessione->query($stringa_query);
							while($riga=$risultato->fetch_assoc())
							{
								echo("<tr><td scope='row'>".$riga['Data']."</td><td>".$riga['Ora']."</td><td>".$riga['Nome_candidato']." ".$riga['Cognome_candidato']."</td><td>".$riga['Nome_esaminatore']." ".$riga['Cognome_esminatore']."</td><td>".$riga['Metodo_colloquio']."</td><td>".$riga['Esito']."</td>");
								echo($td=($riga['Esito']=='')?"<td class='no-border'><a href='Esito_colloquio.php?idcolloquio=".$riga['ID_colloquio']."'><i class='fa fa-pencil-square-o'></i></a></td></tr>":"<td class='no-border'></td>");
							}
						?>
					</tbody>
				</table>
